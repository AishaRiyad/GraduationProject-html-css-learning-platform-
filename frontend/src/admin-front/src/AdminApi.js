// frontend/src/admin-front/src/AdminApi.js
...
export async function getRecent() {
  const r = await fetch(`${API}/recent`, { headers: authHeaders() });
...
export async function listUsers(params = {}) {
  ...
  const r = await fetch(`${API}/users${qs ? `?${qs}` : ""}`, { headers: authHeaders() });
...
export async function listLessons(params = {}) {
  const qs = new URLSearchParams(params).toString();
  const r = await fetch(`${API}/lessons${qs ? `?${qs}` : ""}`, { headers: authHeaders() });
...
export async function listCssLessons() {
  const r = await fetch(`${API}/css-lessons`, { headers: authHeaders() });
...
export async function getAnalyticsOverview(){
  const r = await fetch(`${API}/analytics/overview`, { headers: authHeaders() });
  if(!r.ok) throw new Error("overview failed");
  return r.json();
}
export async function getStudentAnalytics(userId){
  const r = await fetch(`${API}/analytics/student/${userId}`, { headers: authHeaders() });
  if(!r.ok) throw new Error("student analytics failed");
  return r.json();
}

export async function listAnalyticsUsers(){
  const r = await fetch(`${API}/analytics/users`, { headers: authHeaders() });
  return okJson(r, "users");
}
export async function getAnalyticsUser(userId){
  const r = await fetch(`${API}/analytics/users/${userId}`, { headers: authHeaders() });
...
// ================== CHAT (admin or any logged-in user) ==================
const CHAT_API = `${ROOT}/api/chat`;

export async function listMyConversations() {
  const r = await fetch(`${CHAT_API}/conversations`, {
    headers: authHeaders(),
  });
